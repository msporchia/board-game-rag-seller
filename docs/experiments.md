...
| 14 | 2026-07-03 | *(measurement, no change)* — **strong-model benchmark, agent arm (SEL-109 / WP1)**: every LLM role answered by Claude Sonnet 5 through the file-exchange responder harness (commit e23c7c0 — built to benchmark any stronger model on the same oracle before wiring it into production); same engine, retrieval, frozen corpus and oracle as row 13 | ChatConversation (responder harness) | qwen2.5:7b agent **0.733** (11/15; run-to-run variance 0.60/0.80/0.87) | **15/15 (1.000)** · convergence 13/13 · turn oracles 5/5 · fallback/turn 0.062→**0.000** · 99 LLM calls vs 81 (more search retries, incl. recovery from one live retrieval 500) · mean turns-to-converge 1.46 | 🚀 above every run of the baseline's variance band: the 3 non-convergences + the `min_games` miss all disappear. Transcript audit: zero turns without a tool call; "cooperativo" uttered only about card-verified co-ops; no undeclared numeric filters spotted (players left null on "tre o quattro"). Caveats: single sim run vs stochastic baseline; filters_ok/proposal_ok oracles not in scope in the agent arm, so (b)/(c) rest on transcript audit, not oracle; responder ≫ any deployable local model — this measures the engine's ceiling ("the 8B is the bottleneck, not the harness"), not a shippable config. First attempt aborted at case 15: >900s responder-coverage gap tripped the reply timeout (fallback pitch fired) — rerun clean with `--timeout 3600`; ops note: keep responder coverage continuous or raise the timeout |
| 15 | 2026-07-04 | *(probe, no change)* — the thin-record story recomputed at **501 scale, current stack** (bge-m3): TM stripped of its description (rule compose, no LLM) vs the shipped enriched record, 3 showcase queries, throwaway 502-point collection | ad-hoc rank probe | nomic era, 50 games: **#45/#47/#47 → #1/#26/#1** | 501 games, bge: stripped **#2/#34/#21** → shipped **#1/#8/#3** (avg #19 → #4) | ✅ the story survives 10× scale and a strong embedder, restated honestly: the NAME alone now saves the name-informative query (#2), enrichment buys the first screen on the thematic ones (#34→#8, #21→#3). Headline numbers updated in the TM walkthrough + README |
| 16 | 2026-07-04 | **Frontier-tier live takes**: the recorder gained `--exchange` (every LLM role via the responder harness) and two of the four archetype sessions were re-recorded on the live 501 index with **Claude Sonnet 5** at the wheel | live sessions (responder harness) | local takes: named title missed, one invented `cooperative=True` filter, 5/12 turns needed the SEL-147 floor | frontier takes: **«Carcassonne» searched verbatim → #1 + expansion upsell; zero invented filters; coop turn composes query + all 3 structured filters unprompted; pitches cite real game signatures** («senza parlare tra voi» = Magic Maze's actual rule) | ✅ the model-bound flaws vanish with the swap alone, live at 501 scale — matches row 14's bench result. Published side-by-side in showcase/live-session.md (local = free/on-device, frontier = how it sells). Remaining 2 archetypes to record post session-limit reset |
