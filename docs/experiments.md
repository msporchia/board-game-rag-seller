...
|-------|---------|----------|-------------|
| Suite scorecard | `docker compose exec -w /app seller-api sh -c "PYTHONPATH=/app python tests/eval.py --suite core --k 5 --pipeline rule"` | Recall@5, Precision@5, err (≈1−AUC) over 12 queries × 50 frozen games | none (rule pipeline is deterministic) |
...
> ⚠️ Bitrot found 2026-07-03: the documented `python -m tests.eval` no longer runs — the
> `tests/eval/` package shadows `tests/eval.py`. Workaround above (`PYTHONPATH=/app python
> tests/eval.py`); proper fix: rename the script (e.g. `tests/eval_suite.py`) and update docs.
